...
A collection of agent skills loaded by Claude Code, Cursor, Codex, and other Agent Skills-compatible harnesses. Skills are organized into buckets. A consuming repo can be wired in with `/connect`.
...
**Promoted skill**:
A skill in `skills/engineering/` or `skills/productivity/`. Promoted skills appear in `README.md`, `README.zh-CN.md`, `.claude-plugin/plugin.json`, and the generated inventory inside `/catalog`. Skills in `misc/`, `in-progress/`, and `deprecated/` are not promoted.
...
**Setup state**:
The file `docs/agents/setup-state.json` in a consuming repo. It is the canonical record that `/connect` has finished (or is being reconfigured). Do not use HTML comments in `CLAUDE.md` / `AGENTS.md` as a substitute.

## Relationships

- An **Issue tracker** holds many **Issues**
- A **Promoted skill** is either **User-invoked** or **Model-invoked**
- `/connect` writes **Setup state** in the consuming repo
