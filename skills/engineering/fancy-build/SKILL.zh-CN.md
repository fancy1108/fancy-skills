...
description: >-
  ...
  门已绿，或用户在执行当前任务清单时使用。design.md / visual.md / 原型 / tasks.md / decisions.md
...
**shape 门**不绿就拒绝：`design.md` + `visual.md`（或无 UI 豁免）、**高保真原型**（或无 UI 豁免）、`tasks.md`、`decisions.md`，**且用户接受了这些草稿**。否则回到 `/fancy-flow` / `/fancy-shape`。
...
3. 对照原型、`design.md`（token）和 `visual.md`（屏幕 + 禁令）实现。状态和字段名对齐 `contract.ts`。不要扩张 mock；已有原型分支优先替换该任务点名的 mock。
