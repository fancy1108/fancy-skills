...
Don't search for "what idea is good". Search for "what is broken". Quote real complaints (**URL this hunt returned** + one line). No invented Reddit posts.

**Do not use Claude Code `WebSearch` or `WebFetch`.** They are Anthropic server tools. On a proxy (`ANTHROPIC_BASE_URL` ≠ api.anthropic.com) they print `Did 0 searches` and `API Error: Content block not found`. Hunt locally instead:

```bash
python3 <this-skill>/scripts/hunt.py "query one" "query two"
```

The script lives next to this `SKILL.md` (installed copies: `.claude/skills/fancy-orient/scripts/hunt.py`). Run **2–3 queries**, then stop. If it errors or returns 0 rows, tell the user. Do **not** retry `WebSearch`. Do **not** fill a pain table from "domain knowledge".
