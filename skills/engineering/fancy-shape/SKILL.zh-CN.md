...
description: >-
  ...
  已有稳定提案、用户在塑造产品，或他们提到 design.md、visual.md、原型、demo.html、拆任务时使用。不要写业务代码。不要跳过视觉方向和可点
  demo。不要用于小 bug。
---
...
**不要一上来**就定技术栈、写 `visual.md`、写 `demo.html`、或拆 `tasks.md`。和 orient 同一条：先给选项和证据，再锁定。
...
- [ ] `specs/<feature>/design.md` — 设计规范（token）。无 UI：跳过
- [ ] `specs/<feature>/visual.md` — 本产品界面契约（屏幕 + 禁令）— 或 `decisions.md` 里明确的**无 UI**豁免
...
这是**核心契约**，和 `visual.md`、`tasks.md` 同级。后面的代码必须对齐它，而不是反过来。
...
| **无 UI** | 后端 / CLI / 没有界面 | 跳过视觉和原型。在 `decisions.md` 写 `Prototype: none (no UI)`。不需要 `design.md` 和 `visual.md` |
...
3. **契约（第 5 或 5.5 步）** — [ARTIFACTS.md](ARTIFACTS.md)。从零：只有情况 1 或 2 写出了 `design.md` **和** `visual.md` 才做。demo → **停，人看，他们要打磨就打磨（有则调用 `taste` / `impeccable`）** → 接受 → `contract.ts` → 测试契约草稿 → `gate_report.md`。已有：四步审计，不重写 UI，等他们接受那些表。无 UI：跳到拆解。
