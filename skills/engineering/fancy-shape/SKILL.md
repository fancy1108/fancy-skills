...
description: >-
  ...
  design.md, visual.md, prototype, demo.html, or splitting tasks. Do not write business
...
**Do not open** by picking a stack, writing `visual.md`, writing `demo.html`, or splitting `tasks.md`. Same rule as orient: show options and evidence, then lock.
...
- [ ] `specs/<feature>/design.md` — design spec (tokens). No-UI: skip
- [ ] `specs/<feature>/visual.md` — this product's UI contract (screens + bans) — or an explicit **no-UI** waiver in `decisions.md`
...
This is a **core contract**, same rank as `visual.md` and `tasks.md`. Later code must match it, not the other way around.
...
| **No UI** | Backend / CLI / no interface | Skip visual + prototype. Write `Prototype: none (no UI)` in `decisions.md`. `design.md` and `visual.md` are not required |
