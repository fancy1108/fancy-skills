...
Dependencies are expressed as **`/skill`-style prose invocation** ("Run the `/fancy-init` skill"), not deep `../other-skill/FILE.md` cross-references. Shared reference docs live inside the skill that owns them; other skills reach that material by invoking the skill, not by linking across folders.
